...
def build_recent_boards_html(recent_boards):
    ...
    for index, board in enumerate(recent_boards, start=1):
        ...
        html.append(
            ...
            '<button type="submit" class="board-delete-btn js-delete-board" title="Borrar tablero">Ã—</button>'
            '</form>'
        ...
        if column_cards:
            for card in column_cards:
                html.append(
                    ...
                    '<button type="submit" class="card-delete-btn js-delete-card" title="Eliminar tarea" draggable="false">Ã—</button>'
                    '</form>'
                    '</div>'
                )
        else:
...
def build_account_content_html(page_title, page_description, user_name, user_initial, user_email, activities):
    ...
    if page_title == 'Gestionar cuenta':
        ...
        hero.append(
            ...
            f'<div class="signature-avatar">{escape(user_initial)}</div><div><strong>{escape(user_name)}</strong><span>{escape(user_email)}</span><span class="muted">Equipo Prodify Â· Madrid</span></div>'
        ...
        hero.append(
            ...
            '<div class="account-page-card wide-card"><h3>Linea de tiempo</h3><div class="timeline"><div class="timeline-item"><span class="timeline-dot"></span><div><strong>Checklist actualizado</strong><span>Tablero Operaciones Â· 09:40</span></div></div><div class="timeline-item"><span class="timeline-dot"></span><div><strong>Nueva tarjeta creada</strong><span>Marketing Â· 08:15</span></div></div><div class="timeline-item"><span class="timeline-dot"></span><div><strong>Columna reordenada</strong><span>Ventas Â· Ayer</span></div></div></div></div>'
            '</div>'
        )
    elif page_title == 'Tarjetas':
        hero.append(
            '<div class="account-grid"><div class="account-page-card"><h3>Tarjetas destacadas</h3><div class="kanban-mini"><div class="kanban-mini-col"><div class="kanban-mini-title">Pendiente</div><div class="kanban-mini-card">Revisar landing</div><div class="kanban-mini-card">Checklist sprint</div></div><div class="kanban-mini-col"><div class="kanban-mini-title">En progreso</div><div class="kanban-mini-card">Campana marzo</div></div><div class="kanban-mini-col"><div class="kanban-mini-title">Listo</div><div class="kanban-mini-card">Actualizar roadmap</div></div></div></div><div class="account-page-card"><h3>Filtros rapidos</h3><div class="pill-row"><span class="pill">Mis tareas</span><span class="pill">Prioridad alta</span><span class="pill">Vencen hoy</span></div></div><div class="account-page-card wide-card"><h3>Backlog prioritario</h3><div class="backlog-list"><div class="backlog-item"><span class="priority high">Alta</span><div><strong>Revisar onboarding</strong><span>UX Team Â· 2 dias</span></div></div><div class="backlog-item"><span class="priority med">Media</span><div><strong>Optimizar reports</strong><span>Ops Â· 5 dias</span></div></div><div class="backlog-item"><span class="priority low">Baja</span><div><strong>Actualizar etiquetas</strong><span>General Â· 1 semana</span></div></div></div></div></div>'
        )
